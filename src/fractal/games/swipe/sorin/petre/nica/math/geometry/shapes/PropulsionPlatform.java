...
public class PropulsionPlatform extends Rectangle {
    ...
    private void handleStrecthingMotionEvent(MotionEvent motionEvent, Displacement touchPoint) {
        ...
        case MotionEvent.ACTION_MOVE:
            Displacement rawStrecthPoint = touchPoint.subtractionVector(getCenter());
    ...
    private void handleStandingMotionEvent(MotionEvent motionEvent, Displacement touchPoint) {
        if (motionEvent.getActionMasked() == MotionEvent.ACTION_DOWN && touchPoint.distanceTo(getCenter()) < 50 && touchPoint.getY() >= getCenter().getY()) {
            status = Status.STRECTHING;
            projectile.velocity.neutralize();
            projectile.acceleration.neutralize();
            projectile.setCenter(getCenter().subtractionVector(new Displacement(0.0, projectile.height / 2 + height / 2)));
            strecthPoint.makeEqualTo(touchPoint.subtractionVector(getCenter()));
        }
    }
    ...
    public void draw(Canvas canvas) {
        super.draw(canvas);
        Displacement springTip = strecthPoint.getTip();
        strecthPoint.draw(canvas);
        canvas.drawCircle(springTip.getX().floatValue(), springTip.getTip().getY().floatValue(), 10, paint);
        canvas.drawLine(springTip.getX().floatValue(), springTip.getY().floatValue(), evaluateLeftTopCorner().getX().floatValue(), evaluateLeftTopCorner().getY().floatValue(), paint);
        canvas.drawLine(springTip.getX().floatValue(), springTip.getY().floatValue(), evaluateRightTopCorner().getX().floatValue(), evaluateRightTopCorner().getY().floatValue(), paint);
    }
