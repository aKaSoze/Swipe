...
public class Displacement extends Vector2D<Displacement> {
    ...
    public Displacement rotateClockWise(Displacement reference, Double rotation) {
        ...
        Double angleToXAxis = clockWiseAngleTo(reference);
    ...
    public Double clockWiseAngleTo(Displacement other) {
        Displacement delta = subtractionVector(other);
        return Math.atan2(delta.y, delta.x);
    }

    public Double clockWiseRadAngleTo(Displacement other) {
        Double angle = Math.toDegrees(clockWiseAngleTo(other));
        if (angle < 0) {
            angle += 360;
        }
        return angle;
